...
def normalize_ts(ts, conv_rate = 1):
    min_t = min(ts)
    return [abs(t-min_t)/conv_rate for t in ts]
...
def n_classification_load_dataset(dataset):
    ...
    node_labels_path = path + "node-labels-" + dataset + "_new.txt"
    label_names_path = path + "label-names-" + dataset + "_new.txt"
